...
def masked_mape_tf(preds, labels, null_val=np.nan):
    ...
    mask /= tf.reduce_mean(mask)
    mask = tf.where(tf.is_nan(mask), tf.zeros_like(mask), mask)
    loss = tf.abs(tf.div(tf.subtract(preds, labels), labels+tf.constant(5e-3, dtype=tf.float32)))
    loss = loss * mask
    loss = tf.where(tf.is_nan(loss), tf.zeros_like(loss), loss)
    return tf.reduce_mean(loss)
...
def masked_mape(scaler, null_val):
    def mape(preds, labels):
        if scaler:
            preds = scaler.inverse_transform(preds)
            labels = scaler.inverse_transform(labels)
        mape = masked_mape_tf(preds=preds, labels=labels, null_val=null_val)
        return mape
